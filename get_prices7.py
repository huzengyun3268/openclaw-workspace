...
for line in lines:
    if '"' not in line:
        debug.append(f"NO_QUOTE: {repr(line)}")
        continue
    try:
        parts = line.split('"')[1].split(',')
        if len(parts) < 10:
            debug.append(f"TOO_SHORT: {line[:50]} len={len(parts)}")
            continue
        code_full = line.split('_')[1].split('"')[0]
        market = code_full[:2]
        code = code_full[2:]
        if code not in stocks:
            debug.append(f"UNKNOWN: {code_full} -> {market}/{code}")
            continue
        name = stocks[code][1]
        now = float(parts[3])
        high = float(parts[4])
        ...
        debug.append(f"OK: {code} {name} now={now}")
    except Exception as e:
        debug.append(f"ERROR: {line[:60]} err={e}")

debug_data = {'debug': debug, 'results': results}
