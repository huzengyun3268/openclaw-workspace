import urllib.request

stocks = {
    '600352': ('sh', '浙江龙盛', 16.52, 12.0),
    '600893': ('sh', '航发动力', 49.184, 42.0),
    '300033': ('sz', '同花顺', 423.488, 280),
    '601168': ('sh', '西部矿业', 26.169, 22.0),
    '831330': ('bj', '普适导航', 20.361, 18.0),
    '600487': ('sh', '亨通光电', 43.998, 38.0),
    '688295': ('sh', '中复神鹰', 37.843, None),
    '920046': ('bj', '亿能电力', 329.553, None),
    '430046': ('bj', '圣博润', 0.478, None),
    '600089': ('sh', '特变电工', 24.765, 25.0),
}

codes_str = ','.join([f"{v[0]}{k}" for k, v in stocks.items()])
url = f'https://hq.sinajs.cn/list={codes_str}'
req = urllib.request.Request(url, headers={
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
    'Referer': 'https://finance.sina.com.cn'
})
r = urllib.request.urlopen(req, timeout=10)
data = r.read().decode('gbk')

lines = data.strip().split('\n')
print("=== 主账户持仓 ===")
for line in lines[:9]:
    # Parse: var hq_str_shXXXXX="name,open,yclose,now,high,low,buy,sell,vol,amount,...,date,time"
    try:
        parts = line.split('"')[1].split(',')
        if len(parts) < 10:
            continue
        code_full = line.split('_')[1].split('"')[0]
        market = code_full[:2]
        code = code_full[2:]
        if code not in stocks:
            continue
        name = parts[0]
        now = float(parts[3])
        high = float(parts[4])
        low = float(parts[5])
        cost = stocks[code][2]
        stop = stocks[code][3]
        pl = (now - cost) / cost * 100
        pl_amt = (now - cost) * 10000
        stop_pct = (stop / cost - 1) * 100 if stop else None
        print(f"{name}({code}): 现价={now} 最高={high} 最低={low}")
        print(f"  成本={cost} 现价涨跌={pl:.1f}% 盈亏额={pl_amt:.0f}万")
        if stop:
            print(f"  止损={stop}({stop_pct:.1f}%)")
        if now <= stop and stop:
            print(f"  ⚠️⚠️⚠️ 已触及止损！")
        elif stop and now < stop * 1.05:
            print(f"  ⚠️ 接近止损！")
        print()
    except Exception as e:
        print(f"Error: {e}")

print("=== 两融账户 ===")
# 0.600089
for line in lines[9:]:
    try:
        parts = line.split('"')[1].split(',')
        if len(parts) < 10:
            continue
        code_full = line.split('_')[1].split('"')[0]
        market = code_full[:2]
        code = code_full[2:]
        if code not in stocks:
            continue
        name = parts[0]
        now = float(parts[3])
        cost = stocks[code][2]
        stop = stocks[code][3]
        pl = (now - cost) / cost * 100
        pl_amt = (now - cost) * 10000
        print(f"{name}({code}): 现价={now} 成本={cost} 涨跌={pl:.1f}% 盈亏={pl_amt:.0f}万")
        if stop:
            print(f"  止损={stop}")
            if now <= stop:
                print(f"  ⚠️⚠️⚠️ 已触及止损！")
    except Exception as e:
        print(f"Error: {e}")
