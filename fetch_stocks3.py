...
for line in raw.strip().split('\n'):
    if not line.strip():
        continue
    # 格式: v_sh600352="1~浙江龙盛~600352~16.95~16.99~...~"
    parts = line.split('~')
    if len(parts) > 32:
        ...
        for key in stocks_info:
            if key.replace('sh', '').replace('sz', '') in code_full:
                code = key
                break
        else:
            continue
        ...
        if code in stocks_info:
            ...
            try:
                ...
                chg_a = float(chg_amt) if chg_amt else 0
                ...
                if price > 0:
                    ...
                    if stop > 0 and price <= stop:
                        flag = ' [止损!]'
                    print(f'{stock_name}({code}) [{account}]: {price}  {chg_p:+.2f}%  盈亏:{pnl:+.0f}元{flag}')
                else:
                    print(f'{stock_name}({code}): 无数据')
            except Exception as e:
                print(f'{stock_name}({code}): 解析错误-{e}')
